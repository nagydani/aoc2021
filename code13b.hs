...
foldIt :: Page -> [Edge] -> Page
foldIt = foldl' foldOnce

showIt :: Page -> String
showIt p =
    let ps = S.elems p
        yMin = minimum $ map fst ps
        yMax = maximum $ map fst ps
        xMin = minimum $ map snd ps
        xMax = maximum $ map snd ps
        cs = map (\y -> zip [yMin..yMax] (repeat y)) [xMin..xMax]
        gs = map (map (\c -> if S.member c p then '#' else '.')) cs
    in unlines gs

main = do
    f <- readFile "input13.txt"
    let (ps, es) = readIt $ parse input "" f
    putStr $ showIt $ foldIt ps es
